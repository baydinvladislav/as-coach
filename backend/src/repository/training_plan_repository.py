...
class TrainingPlanRepository:
    ...
    async def provide_training_plan_by_id(self, uow: AsyncSession, pk: str) -> TrainingPlanDtoSchema | None:
        query = (
            select(TrainingPlan)
            .where(TrainingPlan.id == UUID(pk))
            .options(
        ...
        def map_diet_to_dto(diet):
            return DietDtoSchema(id=diet.id, proteins=diet.proteins, fats=diet.proteins, carbs=diet.carbs)
